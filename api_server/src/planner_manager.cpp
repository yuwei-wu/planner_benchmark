...
namespace api_server
{
  ...
  void PlannerManager::initPlanModules(rclcpp::Node::SharedPtr &node, ego_planner::PlanningVisualization::Ptr vis)
  {
    ...
    bspline_optimizer_.reset(new ego_planner::BsplineOptimizer);
                                        ...
                                        Eigen::Vector3d local_target_vel, bool flag_polyInit, bool flag_randomPolyTraj)
  {
    ...
    ***/
      ...
      else // Initial path generated from previous trajectory.
      {
        ...
        if (poly_time > ts)
        {
          ...
          for (t = ts; t < poly_time; t += ts)
          {
            ...
            {
              RCLCPP_ERROR(rclcpp::get_logger("api_server"), "pseudo_arc_length is empty, return!");
              continous_failures_count_++;
              return false;
            }
          }
    ...
    ego_planner::UniformBspline::parameterizeToBspline(ts, point_set, start_end_derivatives, ctrl_pts);
    ...
    {
      // cout << "enter" << endl;
      std::vector<ego_planner::ControlPoints> trajs = bspline_optimizer_->distinctiveTrajs(segments);
      cout << "\033[1;33m"
    ...
    ego_planner::UniformBspline pos = ego_planner::UniformBspline(ctrl_pts, 3, ts);
    ...
    else
    {
      ...
      {
        print_once = false;
        RCLCPP_ERROR(rclcpp::get_logger("api_server"), "IN SWARM MODE, REFINE DISABLED!");
      }
    }
  ...
  bool PlannerManager::EmergencyStop(Eigen::Vector3d stop_pos)
  {
    ...
    updateTrajInfo(ego_planner::UniformBspline(control_points, 3, 1.0), rclcpp::Clock().now());

    return true;
  }

  bool PlannerManager::checkCollision(int drone_id)
  {
  ...
  bool PlannerManager::refineTrajAlgo(ego_planner::UniformBspline &traj, vector<Eigen::Vector3d> &start_end_derivative, double ratio, double &ts, Eigen::MatrixXd &optimal_control_points)
  {
    double t_inc;
  ...
  void PlannerManager::updateTrajInfo(const ego_planner::UniformBspline &position_traj, const rclcpp::Time time_now)
  {
  ...
  void PlannerManager::reparamBspline(ego_planner::UniformBspline &bspline, vector<Eigen::Vector3d> &start_end_derivative, double ratio,
